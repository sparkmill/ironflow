...
- Explicit error types via `thiserror` and `Result<T, Error>` — no panics in production code paths.
- Events, inputs, and effects use `#[serde(tag = "type")]` for stable serialization; new event fields use `#[serde(default)]` for backward compatibility.
- Migrations are additive only (new columns/tables/indexes — no drops or renames). Unsafe schema changes need coordination; see [CONTRIBUTING.md](CONTRIBUTING.md#unsafe-schema-changes).
